    ...
    @Query(value = """
            with stats as (select abt.program_id                                                                                                                              as program_id,
                           ...
                           where abt.program_id = :programId
                             ...
                             and (cast(:search as text) is null or p.name ilike '%' || :search || '%' or pgm.name ilike '%' || :search || '%')
            ...
            select s.program_id                                                    as program_id,
                   ...
                   s.date                                                          as date,
                   sum(s.total_received - s.total_granted)
                   over (partition by s.program_id, s.currency_id order by s.date) as total_available,
