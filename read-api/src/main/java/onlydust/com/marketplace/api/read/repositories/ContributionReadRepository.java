...
public interface ContributionReadRepository extends Repository<ContributionReadEntity, Long> {
    ...
    @Query(value = """
            SELECT c.contribution_id                                                                          as contribution_id,
                   ...
                   jsonb_agg(contributor.contributor) filter ( where contributor.contributor_id is not null ) as contributors,
                   jsonb_agg(applicant.contributor) filter ( where applicant.contributor_id is not null )     as applicants,
    ...
    default Page<ContributionReadEntity> findAll(ContributionsQueryParams q) {
        return findAll(
                PageRequest.of(q.getPageIndex(), q.getPageSize())
        );
    }
}
