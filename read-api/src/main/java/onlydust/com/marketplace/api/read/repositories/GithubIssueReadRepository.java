...
public interface GithubIssueReadRepository extends Repository<GithubIssueReadEntity, Long> {
    ...
    @Query(value = """
            ...
            FROM indexer_exp.github_issues i
                     ...
                     LEFT JOIN hackathon_projects hp ON hp.project_id = pgr.project_id
                     LEFT JOIN hackathons h ON gl_hackathon.name = ANY (h.github_labels) AND hp.hackathon_id = h.id
