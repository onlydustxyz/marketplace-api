...
public record RewardCanceledDTO(@NonNull String amount, @NonNull String currency, @NonNull String rewardName, @NonNull String username) {
    ...
    public static RewardCanceledDTO fromEvent(@NonNull String recipientLogin, @NonNull RewardCanceled rewardCanceled) {
        return new RewardCanceledDTO(rewardCanceled.shortReward().getAmount().setScale(3, RoundingMode.HALF_UP).toString(),
                rewardCanceled.shortReward().getCurrencyCode(), "%s - %s".formatted(rewardCanceled.shortReward().getId().pretty(),
                rewardCanceled.shortReward().getProjectName()), recipientLogin);
    }
}
