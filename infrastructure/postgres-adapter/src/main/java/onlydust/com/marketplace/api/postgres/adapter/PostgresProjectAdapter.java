...
public class PostgresProjectAdapter implements ProjectStoragePort {
  ...
  public ProjectRewardsPageView findRewards(UUID projectId, ProjectRewardView.Filters filters,
    ...
    return ProjectRewardsPageView.builder().
        ...
        .sentRewardsCount(
            budgetStats.stream().map(BudgetStatsEntity::getRewardIds).flatMap(Collection::stream).collect(Collectors.toUnmodifiableSet()).size())
        .rewardedContributionsCount(
            budgetStats.stream().map(BudgetStatsEntity::getRewardItemIds).flatMap(Collection::stream).collect(Collectors.toUnmodifiableSet()).size())
        .rewardedContributorsCount(
            budgetStats.stream().map(BudgetStatsEntity::getRewardRecipientIds).flatMap(Collection::stream).collect(Collectors.toUnmodifiableSet())
                .size())
        .build();
  }
