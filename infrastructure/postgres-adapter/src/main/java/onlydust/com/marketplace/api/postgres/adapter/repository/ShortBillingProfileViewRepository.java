...
public interface ShortBillingProfileViewRepository extends JpaRepository<ShortBillingProfileViewEntity, UUID> {
    ...
    @Query(value = """
            select bp.*, bpu.role, false pending_invitation from accounting.billing_profiles bp
    ...
    @Query(value = """
            select bp.*, bpu.role, false pending_invitation
            ...
            union
            select bp2.*,bpui.role, true pending_invitation
            from iam.users u
