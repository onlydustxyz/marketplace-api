...
public interface HistoricalTransactionRepository extends JpaRepository<HistoricalTransactionEntity, UUID> {
    @Query(value = """
            ...
            """,
            countQuery = """
                    WITH deposits AS (SELECT sa.sponsor_id, count(*) as count
                                      FROM accounting.sponsor_accounts sa
                                               JOIN accounting.sponsor_account_transactions sat
                                                    ON sat.account_id = sa.id AND sat.type = 'DEPOSIT'
                                      GROUP BY sa.sponsor_id),
                         spends as (SELECT sa.sponsor_id, count(*) as count
                                    FROM accounting.sponsor_accounts sa
                                             JOIN accounting.account_books_events abe
                                                  ON CAST(abe.payload #>> '{event, from, id}' AS UUID) = sa.id AND
                                                     abe.payload #>> '{event, @type}' = 'Transfer' AND
                                                     abe.payload #>> '{event, to, type}' = 'PROJECT'
                                             JOIN accounting.account_books ab on ab.id = abe.account_book_id
                                    GROUP BY sa.sponsor_id)
                    SELECT d.count + s.count
                    FROM deposits d
                             JOIN spends s ON d.sponsor_id = s.sponsor_id
                    WHERE d.sponsor_id = :sponsorId
                    """, nativeQuery = true)
