...
create table if not exists indexer_exp.github_accounts
(
    id              bigint                          not null
        primary key,
    login           text                            not null,
...
create unique index if not exists github_accounts_login_uidx
...
create table if not exists indexer_exp.github_app_installations
(
    id              bigint                  not null
        primary key,
...
create unique index if not exists github_app_installations_account_id_idx
...
create table if not exists indexer_exp.github_repos
(
    id              bigint                             not null
        primary key,
...
create table if not exists indexer_exp.authorized_github_repos
(
...
create table if not exists indexer_exp.github_issues
(
    id                bigint                          not null
        primary key,
    repo_id           bigint                          not null
...
create unique index if not exists github_issues_id_as_text_index
...
create unique index if not exists github_issues_contribution_uuid_index
...
create table if not exists indexer_exp.github_issues_assignees
(
...
create table if not exists indexer_exp.github_pull_requests
(
    id                   bigint                                       not null
        primary key,
    repo_id              bigint                                       not null
...
create table if not exists indexer_exp.github_code_reviews
(
    id                text                                 not null
        primary key,
...
create table if not exists indexer_exp.contributions
(
    id                       text                            not null
        primary key,
    repo_id                  bigint                          not null
...
create index if not exists contributions_completed_at_created_at_idx
    on indexer_exp.contributions (COALESCE(completed_at, created_at));

create index if not exists contributions_completed_at_created_at_desc_idx
    on indexer_exp.contributions (COALESCE(completed_at, created_at) desc);

create index if not exists contributions_github_author_id_index
    on indexer_exp.contributions (github_author_id);

create index if not exists contributions_pull_request_id_index
    on indexer_exp.contributions (pull_request_id);

create index if not exists indexer_exp_contributions_contributor_id_created_at_idx
    on indexer_exp.contributions (contributor_id, created_at);

create index if not exists contributions_contribution_uuid_index
...
create unique index if not exists github_code_reviews_contribution_uuid_index
...
create table if not exists indexer_exp.github_pull_request_commit_counts
(
    pull_request_id bigint  not null
        references indexer_exp.github_pull_requests
...
create unique index if not exists github_pull_requests_id_as_text_index
    on indexer_exp.github_pull_requests ((id::text));

create unique index if not exists github_pull_requests_contribution_uuid_index
...
create table if not exists indexer_exp.github_pull_requests_closing_issues
(
    pull_request_id bigint not null
        references indexer_exp.github_pull_requests,
    issue_id        bigint not null
...
create unique index if not exists github_pull_requests_closing_issues_issue_id_idx
    on indexer_exp.github_pull_requests_closing_issues (issue_id, pull_request_id);

create table if not exists indexer_exp.github_repo_languages
(
    repo_id         bigint                  not null
        references indexer_exp.github_repos,
...
create index if not exists github_repos_owner_login_name_idx
    on indexer_exp.github_repos (owner_login, name);

create index if not exists github_repos_id_visibility_idx
...
create table if not exists indexer_exp.github_repos_stats
(
    id              bigint not null
        primary key
        references indexer_exp.github_repos,
...
create table if not exists indexer_exp.repos_contributors
(
    repo_id                      bigint  not null
        references indexer_exp.github_repos,
...
create table if not exists indexer_exp.github_labels
(
    id              bigint                  not null
        primary key,
    name            text                    not null,
...
create table if not exists indexer_exp.github_issues_labels
(
    issue_id        bigint                  not null
        references indexer_exp.github_issues,
...
create table if not exists indexer_exp.github_user_file_extensions
(
    user_id            bigint                                 not null,
...
create table if not exists indexer_exp.github_commits
(
    sha             text                                   not null
        primary key,
    author_id       bigint,
...
create table if not exists indexer_exp.github_pull_requests_commits
(
...
create table if not exists indexer_exp.grouped_contributions
(
    contribution_uuid        uuid                            not null
        primary key,
    repo_id                  bigint                          not null
...
create index if not exists grouped_contributions_code_review_id_as_text_index
    on indexer_exp.grouped_contributions (code_review_id);

create index if not exists grouped_contributions_completed_at_created_at_desc_idx
    on indexer_exp.grouped_contributions (COALESCE(completed_at, created_at) desc);

create index if not exists grouped_contributions_completed_at_created_at_idx
    on indexer_exp.grouped_contributions (COALESCE(completed_at, created_at));

create index if not exists grouped_contributions_completed_at_index
    on indexer_exp.grouped_contributions (completed_at);

create index if not exists grouped_contributions_created_at_index
    on indexer_exp.grouped_contributions (created_at);

create index if not exists grouped_contributions_github_author_id_index
    on indexer_exp.grouped_contributions (github_author_id);

create index if not exists grouped_contributions_pull_request_id_index
    on indexer_exp.grouped_contributions (pull_request_id);

create index if not exists grouped_contributions_repo_id_index
    on indexer_exp.grouped_contributions (repo_id);

create index if not exists grouped_contributions_status_as_text_index
    on indexer_exp.grouped_contributions (status);

create index if not exists grouped_contributions_type_as_text_index
    on indexer_exp.grouped_contributions (type);

create index if not exists grouped_contributions_coalesce_idx
...
create table if not exists indexer_exp.grouped_contribution_contributors
(
    contribution_uuid uuid                    not null
...
create unique index if not exists grouped_contribution_contributors_pk_inv
