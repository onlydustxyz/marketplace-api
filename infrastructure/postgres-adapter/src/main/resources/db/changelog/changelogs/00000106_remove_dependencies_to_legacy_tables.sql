              ...
              from project_github_repos pgr2
                       join indexer_exp.github_repos gr2
                            on gr2.id = pgr2.github_repo_id and
                               pgr2.project_id = pd2.project_id and
                               gr2.visibility = 'PUBLIC'));


ALTER TABLE reward_to_batch_payment
    DROP CONSTRAINT reward_to_batch_payment_reward_id_fkey,
    ADD FOREIGN KEY (reward_id) REFERENCES rewards (id);


ALTER TABLE rewards
    DROP CONSTRAINT rewards_project_id_fkey,
    ADD FOREIGN KEY (project_id) REFERENCES project_details (project_id);

ALTER TABLE pending_project_leader_invitations
    DROP CONSTRAINT pending_project_leader_invitations_project_id_fkey,
    ADD FOREIGN KEY (project_id) REFERENCES project_details (project_id);

ALTER TABLE project_allowances
    DROP CONSTRAINT project_allowances_project_id_fkey,
    ADD FOREIGN KEY (project_id) REFERENCES project_details (project_id);

ALTER TABLE projects_sponsors
    DROP CONSTRAINT projects_sponsors_project_id_fkey,
    ADD FOREIGN KEY (project_id) REFERENCES project_details (project_id);

ALTER TABLE projects_ecosystems
    DROP CONSTRAINT projects_ecosystems_project_id_fkey,
    ADD FOREIGN KEY (project_id) REFERENCES project_details (project_id);

ALTER TABLE reward_items
    DROP CONSTRAINT reward_items_project_id_fkey,
    ADD FOREIGN KEY (project_id) REFERENCES project_details (project_id);
