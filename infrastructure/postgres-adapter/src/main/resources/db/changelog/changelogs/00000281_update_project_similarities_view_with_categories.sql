drop view project_similarities;

create view project_similarities as
with similar_languages as (select p1.project_id                      as project1_id,
     ...
     projects_similarity_ranks as (select coalesce(sl.project1_id, sc.project1_id, se.project1_id)        as project1_id,
                                          ...
                                          rank() over (order by array_length(sl.similar_languages, 0))    as similar_language_count_rank,
                                          rank() over (order by array_length(sc.similar_contributors, 0)) as similar_contributor_count_rank,
                                          rank() over (order by array_length(se.similar_ecosystems, 0))   as similar_ecosystem_count_rank,
                                          rank() over (order by array_length(sca.similar_categories, 0))  as similar_category_count_rank,
