...
CREATE MATERIALIZED VIEW bi_internal.contribution_contributor_timestamps AS
SELECT DISTINCT c.created_at     as timestamp,
                c.contributor_id as contributor_id
FROM completed_contributions c
;
create unique index bi_contribution_contributor_timestamps_pk on bi_internal.contribution_contributor_timestamps (contributor_id, timestamp);
create unique index bi_contribution_contributor_timestamps_pk_inv on bi_internal.contribution_contributor_timestamps (timestamp, contributor_id);
...
select c.contribution_id,
       ...
       c.is_merged_pr,
       c.previous_project_contribution_timestamp,
       c.next_project_contribution_timestamp,
       c.previous_contributor_contribution_timestamp,
       c.next_contributor_contribution_timestamp
