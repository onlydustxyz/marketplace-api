       ...
       coalesce(sum(rsd.amount_usd_equivalent)
                FILTER (WHERE
                    (rsd.paid_at BETWEEN date_trunc('year'::text, now()) AND (date_trunc('year'::text, now()) + '1 year'::interval))
                        OR (rsd.invoice_received_at IS NOT NULL AND rsd.paid_at IS NULL)), 0) AS yearly_usd_total
FROM accounting.billing_profiles bp
         LEFT JOIN rewards r on r.billing_profile_id = bp.id
         LEFT JOIN accounting.reward_status_data rsd ON r.id = rsd.reward_id
GROUP BY bp.id;
...
SELECT reward_id,
       CASE
           ...
           WHEN s.is_individual IS TRUE AND s.current_year_usd_total + s.reward_usd_equivalent > 5000
               THEN 'PAYMENT_BLOCKED'::accounting.reward_status
