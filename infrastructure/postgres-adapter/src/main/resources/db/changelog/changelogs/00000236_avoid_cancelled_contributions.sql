...
with ranks as (select coalesce(c.github_user_id, rr.github_user_id, rs.github_user_id) as github_user_id,
                      ...
                      cume_dist() over (order by c.rank)                               as contributions_cume_dist,
                      cume_dist() over (order by rs.rank)                              as rewards_sent_cume_dist,
                      cume_dist() over (order by rr.rank)                              as rewards_received_cume_dist,
     ...
     normalized_ranks as (select r.github_user_id                                                     as github_user_id,
                                 ...
                                 100 * r.contributions_cume_dist                                      as contributions_cume_dist,
                                 100 * r.rewards_sent_cume_dist                                       as rewards_sent_cume_dist,
                                 100 * r.rewards_received_cume_dist                                   as rewards_received_cume_dist,
...
select rank() over (order by 100 * r.contributions_rank +
                ...
                as rank,
       cume_dist() over (order by 100 * r.contributions_cume_dist +
                                  2 * r.rewards_sent_cume_dist +
                                  1.5 * r.rewards_received_cume_dist)
                as rank_percentile,
       r.*,
