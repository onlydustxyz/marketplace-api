...
create view project_recommendations as
with similar_languages as (select pl.project_id as project_id,
                                  upi.id        as user_id,
                                  array_agg(ul) as languages,
                                  count(ul)     as language_count
                           from user_profile_info upi,
                                unnest(upi.preferred_language_ids) ul(ul)
                                    left join project_languages pl on pl.language_id = ul
                           group by pl.project_id, upi.id),
