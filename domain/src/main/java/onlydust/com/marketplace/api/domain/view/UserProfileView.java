...
public class UserProfileView {
  ...
  public void addProjectStats(final ProjectStats projectStats) {
    if (projectStats.getUserFirstContributedAt() != null && (firstContributedAt == null || projectStats.getUserFirstContributedAt()
        .before(firstContributedAt))) {
  ...
  public static class ProfileStats {
    ...
    public int getContributionCountVariationSinceLastWeek() {
      ...
      final var currentWeekWithStats = contributionStats.stream()
          .filter(stats -> stats.getYear() == currentWeek.getYear() && stats.getWeek() == currentWeek.get(
              WeekFields.of(Locale.getDefault()).weekOfYear())).findFirst();
      final var previousWeekWithStats = contributionStats.stream()
          .filter(stats -> stats.getYear() == previousWeek.getYear() && stats.getWeek() == previousWeek.get(
              WeekFields.of(Locale.getDefault()).weekOfYear())).findFirst();
