...
public class ProjectsPostRewardableOtherWorkApiIT extends AbstractMarketplaceApiIT {
  ...
  public void should_be_unauthorized() {
    // When
    client.post().uri(getApiURI(String.format(PROJECTS_POST_REWARDABLE_OTHER_WORK, UUID.randomUUID()))).contentType(APPLICATION_JSON)
  ...
  void should_be_forbidden_given_authenticated_user_project_lead_on_not_linked_repo() {
    ...
    // When
    client.post().uri(getApiURI(String.format(PROJECTS_POST_REWARDABLE_OTHER_WORK, projectId))).contentType(APPLICATION_JSON)
        .bodyValue(String.format("""
            {
        ...
        // Then
        .exchange().expectStatus().isEqualTo(403).expectBody().jsonPath("$.message")
        .isEqualTo("Rewardable " + "issue can " + "only be " + "created on " + "repos linked " + "to this " + "project");
  }
