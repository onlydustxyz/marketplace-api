...
public class ProjectsPostRewardableOtherPullRequestApiIT extends AbstractMarketplaceApiIT {
    ...
    public void should_be_unauthorized() {
        // When
        client.post().uri(getApiURI(port, String.format(PROJECTS_POST_REWARDABLE_OTHER_PR, UUID.randomUUID()))).contentType(APPLICATION_JSON).bodyValue("""
                        {
    ...
    void should_be_forbidden_given_authenticated_user_not_project_lead() {
        ...
        // When
        client.post().uri(getApiURI(port, String.format(PROJECTS_POST_REWARDABLE_OTHER_PR, projectId))).contentType(APPLICATION_JSON).bodyValue("""
                        {
                          "githubPullRequestHtmlUrl": "https://github.com/golang/go/pulls/64179"
                        }
    ...
    void should_create_and_close_rewardable_issue_given_a_project_lead_and_linked_repo() {
        ...
        client.post().uri(getApiURI(port, String.format(PROJECTS_POST_REWARDABLE_OTHER_PR, projectId)))
